...
public:
  ...
  /**
   * @brief 获取指定按键的动作状态（供外部类如Launcher访问protected的key数组）
   * @param _idx 按键索引（KEY_0或KEY_1）
   * @return 按键动作：RELEASE=松开, CLICK=短按, PRESS=长按
   */
  static key::keyAction getKeyAction(key::KEY_INDEX _idx) { return get()->key[_idx]; }

  /**
   * @brief 清除所有按键动作状态（处理完按键事件后调用，防止重复触发）
   */
  static void clearKeyAction() { memset(get()->key, key::RELEASE, sizeof(get()->key)); }
