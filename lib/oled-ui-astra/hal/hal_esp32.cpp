...
static void IRAM_ATTR encoderISR() {
  bool clk = digitalRead(ENCODER_CLK);  // 读取当前CLK电平
  bool dt = digitalRead(ENCODER_DT);    // 读取当前DT电平
  if (clk != _lastCLK) {                // CLK发生了变化（上升沿或下降沿）
    if (dt != clk) _encPos++;           // DT与CLK不同相 -> 右转
    else _encPos--;                     // DT与CLK同相 -> 左转
  }
  _lastCLK = clk;                       // 保存当前CLK状态供下次比较
}
...
void HALESP32::_keyScan() {
  ...
  switch (keyFilter) {
    case key::CHECKING:
      // 等待状态：检测是否有输入
      if (anyInput) {
        if (leftTurn) keyFilter = key::KEY_0_CONFIRM;       // 左转 -> 确认KEY_0
        else if (rightTurn || btnPressed) keyFilter = key::KEY_1_CONFIRM; // 右转或按钮 -> 确认KEY_1
      }
      timeCnt = 0;
      lock = false;
      break;
